...
class Planner:
    ...
    def get_weekly_plan(self):
        ...
        Each task appears on its deadline day if within the week.
        Tasks due beyond this week appear on the last day of the week as a heads-up.
        """
        plan = {}
        for i in range(7):
        ...
        for task in pending:
            if task.deadline in plan:
                # deadline falls within this week — show on that day
                plan[task.deadline].append(task)
            elif task.days_until_due() > 0:
                # deadline is beyond this week — show on last day as a reminder
    ...
    if not subject_names:
        ...
        if st.session_state.show_add_task:
            ...
            with st.form("add_task_form", clear_on_submit=True):
                ...
                est_hours = st.number_input("Estimated hours", min_value=1, max_value=100, value=2, step=1)
                notes = st.text_area("Notes", placeholder="Anything relevant — links, requirements, reminders...", height=80)
                ...
                if submitted:
                    if not title.strip():
                        ...
                        st.session_state.tasks.append(
                            Task(title.strip(), subject, deadline, priority, tasktype, est_hours, notes.strip())
                        )
                        save_data()
                        st.session_state.show_add_task = False
                        st.rerun()
...
def page_weekly_plan(planner):
    ...
    for day, day_tasks in plan.items():
        ...
        # split tasks for last day: due that day vs beyond-week reminders
        due_today_tasks   = []
        reminder_tasks    = []
        if is_last:
            for t in day_tasks:
        ...
        count_str = f"{len(all_display)} task{'s' if len(all_display) != 1 else ''}" \
                    if all_display else "free"
        ...
        if not all_display and not reminder_tasks:
            ...
            if reminder_tasks:
                st.markdown(
                    "<div style='font-size:11px;color:#9b9a97;margin-top:10px;margin-bottom:4px'>"
                    "Coming up after this week</div>",
                    unsafe_allow_html=True,
                )
