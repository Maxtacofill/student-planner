...
def apply_styles():
    ...
    <style>
        ...
        /* prevent expander SVG icon from rendering as "_arrow_right" text on mobile */
        [data-testid="stExpander"] summary svg title,
        [data-testid="stExpander"] summary svg desc,
        [data-testid="stExpander"] summary span[aria-hidden] {
...
def page_tasks(planner):
    ...
    if completed_tasks:
        ...
        st.markdown(
            f"<div style='font-size:11px;font-weight:600;letter-spacing:0.07em;"
            f"text-transform:uppercase;color:#9b9a97;margin-bottom:10px'>"
            f"📦 Completed tasks ({len(completed_tasks)})</div>"
            f"<div style='font-size:12px;color:#9b9a97;margin-bottom:10px'>"
            f"These tasks are done. Mark them incomplete using the checkbox if needed.</div>",
            unsafe_allow_html=True,
        )
        for task in completed_tasks:
            subj  = planner.get_subject(task.subject_name)
            color = subj.color if subj else "#888"
            col_chk, col_card, col_del = st.columns([0.4, 7, 1.2])
            new_val = col_chk.checkbox("done", value=task.completed,
...
def page_weekly_plan(planner):
    ...
    st.markdown(
        "<div style='background:#f9f9f8;border:1px solid #ebebea;border-radius:8px;"
        "padding:12px 16px;margin-bottom:12px;font-size:12px;color:#787774;line-height:1.7'>"
        "<b style='color:#1a1a1a'>ℹ️ How scoring works</b> — "
        "Urgency = (priority weight × 10) ÷ days until deadline. "
        "Weights: High = 3 · Medium = 2 · Low = 1. Overdue = 100. "
        "The plan spreads work backwards from each deadline in sessions of your chosen length, "
        "capped at your daily max. Pomodoro sessions done are subtracted automatically."
        "</div>",
        unsafe_allow_html=True,
    )
...
def page_subjects(planner):
    ...
    for subject in st.session_state.subjects:
        ...
        done_badge = "<span style='font-size:11px;color:#2d7a3a;font-weight:500;margin-left:6px'>✓ All done</span>" if is_done else ""

        # render name + colour dot + progress as one HTML block
        st.markdown(
            f"<div style='display:flex;align-items:center;gap:10px;margin-bottom:4px'>"
            f"<div style='width:12px;height:12px;min-width:12px;border-radius:50%;"
            f"background:{subject.color}'></div>"
            f"<div style='font-size:15px;font-weight:600;color:#1a1a1a'>"
            f"{subject.name}{done_badge}</div>"
            f"</div>"
            f"<div style='background:#e8e8e4;border-radius:4px;height:6px;margin-bottom:4px'>"
            f"<div style='width:{bar_pct}%;background:{bar_color};border-radius:4px;height:6px'></div>"
            f"</div>"
            f"<div style='font-size:12px;color:#787774;margin-bottom:8px'>"
            f"{s['completed']}/{s['total']} tasks · {bar_pct}%</div>",
            unsafe_allow_html=True,
        )

        # edit + delete as a proper row — never stacks
        btn_col1, btn_col2, spacer = st.columns([1, 1, 4])
        if btn_col1.button("✏️ Edit", key=f"editsub_{subject.name}", use_container_width=True):
            st.session_state.editing_subject = subject
            st.rerun()
        if btn_col2.button("🗑 Delete", key=f"delsub_{subject.name}", use_container_width=True):
...
def page_profile_select():
    ...
    if profiles:
        ...
        # search if more than 4 profiles
        if len(profiles) > 4:
            search_query = st.text_input("🔍 Search profiles", placeholder="Type a name...",
                                         label_visibility="collapsed")
            filtered = [p for p in profiles if search_query.lower() in p.lower()]
        else:
            filtered = profiles
        ...
        for name in filtered:
            ...
            # PIN entry appears immediately under THIS profile when selected
            if st.session_state.pin_for == name:
                st.markdown(
                    f"<div style='font-size:13px;color:#9b9a97;margin:4px 0 6px'>"
                    f"Enter PIN for <b style='color:#1a1a1a'>{name}</b></div>",
                    unsafe_allow_html=True,
                )
                with st.form(f"pin_form_{name}"):
                    ...
                    if c1.form_submit_button("Enter", use_container_width=True):
                        ...
                        if stored_pins.get(name) == pin_input:
                            st.session_state.current_user = name
                            st.session_state.pin_for = None
                            saved_subjects, saved_tasks = load_data(name)
                            st.session_state.subjects = saved_subjects
                            st.session_state.tasks    = saved_tasks
                            st.rerun()
                        else:
...
def main():
    ...
    # ── mobile nav — rendered FIRST so it appears at top on mobile ───────────
    ...
    with st.sidebar:
        ...
        sidebar_page = st.radio(
            ...
            ["🏠  Dashboard", "✅  Tasks", "📅  Weekly plan",
             "📚  Subjects", "📊  Progress report"],
            index=["🏠  Dashboard", "✅  Tasks", "📅  Weekly plan",
                   "📚  Subjects", "📊  Progress report"].index(page),
            label_visibility="hidden",
        )
        # on desktop the sidebar radio takes priority
        page = sidebar_page
