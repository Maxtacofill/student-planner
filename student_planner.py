...
def apply_styles():
    ...
    <style>
        ...
        /* hide ALL text/spans inside dropzone instructions so only button shows */
        [data-testid="stFileUploaderDropzoneInstructions"] {
            display: none !important;
        }
        [data-testid="stFileUploader"] > div > div > div:first-child > span,
        [data-testid="stFileUploader"] > div > div > div:first-child > small {
            display: none !important;
        }
    ...
    if profiles:
        ...
        for name in profiles:
            ...
            st.markdown(
                ...
                f"padding:12px 14px;margin-bottom:6px;background:{bg_color};cursor:pointer'>",
                unsafe_allow_html=True,
            )
        ...
        if st.session_state.pin_for is not None:
            ...
            st.markdown(
                f"<div style='border:1px solid #ebebea;border-radius:8px;"
                f"padding:16px 18px;background:#f9f9f8;margin:8px 0 16px'>",
                unsafe_allow_html=True,
            )
            st.markdown(
                f"<div style='font-size:13px;color:#1a1a1a;margin-bottom:10px'>"
                f"Enter PIN for <b>{target}</b></div>",
                unsafe_allow_html=True,
            )
