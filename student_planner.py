...
def apply_styles():
    ...
    <style>
        ...
        /* eye toggle button — force white bg, dark icon */
        ...
        @media (max-width: 768px) {
            ...
            /* ── sidebar: collapse by default, full-width when open ── */
            ...
            /* ── task/day cards: let text wrap ── */
            .task-card, [data-testid="stMarkdownContainer"] div {
            ...
            /* ── hide the sidebar collapse arrow on mobile (confusing) ── */
            [data-testid="collapsedControl"] {
                display: flex !important;
                z-index: 999 !important;
            }

            /* ── profile page: center and pad ── */
        ...
        /* ═══════════════════════════════════════════════════════════════════
           SMALL PHONE  (≤ 480px) — extra tweaks
           ═══════════════════════════════════════════════════════════════════ */
        @media (max-width: 480px) {
            ...
            /* sidebar nav items bigger for thumb tapping */
...
def page_profile_select():
    ...
    if profiles:
        ...
        for name in profiles:
            ...
            border_color = "#1a1a1a" if is_selected else "#ebebea"
            bg_color = "#f9f9f8" if is_selected else "#ffffff"

            col_av, col_name, col_btn = st.columns([0.5, 4, 1.5])
            col_av.markdown(
                f"<div style='width:36px;height:36px;border-radius:50%;"
                f"background:#1a1a1a;color:#fff;display:flex;align-items:center;"
                f"justify-content:center;font-size:15px;font-weight:600;line-height:36px;"
                f"text-align:center;margin-top:4px'>{initial}</div>",
                unsafe_allow_html=True,
            )
            col_name.markdown(
                f"<div style='font-size:14px;font-weight:500;color:#1a1a1a;"
                f"padding-top:10px'>{name}</div>",
                unsafe_allow_html=True,
            )
            if col_btn.button("Select", key=f"profile_{name}", use_container_width=True):
                st.session_state.pin_for = name
                st.rerun()
            st.markdown(
                f"<div style='border-top:1px solid {border_color};margin:2px 0 10px'></div>",
                unsafe_allow_html=True,
            )
