...
class Task:
    ...
    def __init__(self, title, subject_name, deadline, priority, task_type):
        if priority not in self.PRIORITIES:
            raise ValueError(f"priority must be one of {self.PRIORITIES}")
        if task_type not in self.TYPES:
            raise ValueError(f"task_type must be one of {self.TYPES}")
    ...
    def __repr__(self):
        return f"Task(title={self.title!r}, deadline={self.deadline})"
...
class Planner:
    ...
    def get_weekly_plan(self):
        """Returns {date: [tasks]} for the next 7 days, sorted by urgency."""
        plan = {}
        for i in range(7):
        ...
        # sort pending by urgency score, highest first (insertion sort style)
        ...
        for task in pending:
            if task.deadline in plan:
                ...
                earliest = list(plan.keys())[0]
                plan[earliest].append(task)
        return plan
...
def apply_styles():
    ...
    <style>
        ...
        [data-testid="stFormSubmitButton"] button:hover { background: #2d2d2d !important; }
...
def page_dashboard(planner):
    ...
    with col_w:
        ...
        if weather:
            st.markdown(
                f"<span style='font-size:13px;color:#787774'>{weather['icon']}</span> "
                f"<span style='font-size:20px;font-weight:600;color:#1a1a1a'>{weather['temp']}°C</span> "
                f"<span style='color:#787774;font-size:14px'> {weather['condition']}, "
                f"wind {weather['wind']} km/h</span>",
                unsafe_allow_html=True,
            )
            st.caption(weather["tip"])
        else:
            st.caption("Weather data unavailable. Check your connection.")

    with col_h:
        st.markdown("**Upcoming holidays (Portugal)**")
        holidays = st.session_state.holidays
        if holidays:
            for h in holidays[:5]:
                d = date.fromisoformat(h["date"])
                st.markdown(f"**{h['name']}** &nbsp;·&nbsp; {d.strftime('%d %b %Y')}",
                            unsafe_allow_html=True)
        else:
...
def page_tasks(planner):
    ...
    if not subject_names:
        ...
        if st.button("New task", use_container_width=False):
        ...
        if st.session_state.show_add_task:
            ...
            with st.form("add_task_form", clear_on_submit=True):
                title = st.text_input("Task title", placeholder="e.g. Assignment 4 - OOP")
                col1, col2 = st.columns(2)
                subject  = col1.selectbox("Subject", subject_names)
                tasktype = col2.selectbox("Type", Task.TYPES)
                col3, col4 = st.columns(2)
                deadline = col3.date_input("Deadline",
                                           value=date.today() + timedelta(days=7),
                                           min_value=date.today())
                priority = col4.selectbox("Priority", Task.PRIORITIES)
                ...
                if submitted:
                    if not title.strip():
                        ...
                        st.session_state.tasks.append(
                            Task(title.strip(), subject, deadline, priority, tasktype)
                        )
                        st.session_state.show_add_task = False
                        st.rerun()
...
def page_weekly_plan(planner):
    ...
    if weather:
        st.caption(
            f"{weather['condition']}, {weather['temp']}C. {weather['tip']}"
        )

    st.caption(
        "Tasks are ranked by urgency score, a combination of priority level "
        "and days remaining until the deadline. Tasks due beyond this week "
        "appear on the earliest available day as a reminder."
    )
    ...
    for day, day_tasks in plan.items():
        ...
        count_str    = f"{len(day_tasks)} task{'s' if len(day_tasks) != 1 else ''}" \
                       if day_tasks else "free"

        day_label = f"{DAY_NAMES[day.weekday()]} {day.strftime('%d %b')}"
        if is_today:
            day_label = "Today  " + day_label
        if holiday_name:
            day_label = day_label + "  (" + holiday_name + ")"
        ...
        st.markdown(
            ...
            f"margin-bottom:{'10px' if day_tasks or holiday_name else '0'}'>"
            f"{day_label}"
            f"<span style='font-size:12px;font-weight:400;color:#9b9a97;margin-left:10px'>"
            f"{count_str}</span></div>",
            unsafe_allow_html=True,
        )
        if holiday_name:
            st.markdown(
                f"<div style='font-size:12px;color:#a56123;margin-bottom:8px'>"
                f"Public holiday. Libraries may be closed.</div>",
                unsafe_allow_html=True,
            )
        if not day_tasks:
            if holiday_name:
                ...
                st.markdown(
                    "<div style='font-size:12px;color:#9b9a97'>Nothing scheduled.</div>",
                    unsafe_allow_html=True,
                )
        else:
...
def page_progress(planner):
    ...
    # build a plain dict for st.bar_chart — no pandas needed
    chart_data = {}
    for name, data in stats_by_subject.items():
        if data["total"] > 0:
            chart_data[name] = data["pct"]
    if chart_data:
    ...
    for name, data in stats_by_subject.items():
        ...
        color = subj.color if subj else "#888"
        col_dot, col_name, col_bar, col_pct = st.columns([0.3, 2, 4, 1])
        col_dot.markdown(
            f"<div style='width:12px;height:12px;border-radius:50%;"
        ...
        col_bar.markdown(
            f"<div style='margin-top:4px;background:#e8e8e4;border-radius:4px;height:8px;'>"
            f"<div style='width:{data['pct']}%;background:#1a1a1a;border-radius:4px;height:8px;'></div>"
            f"</div>"
            f"<div style='font-size:11px;color:#787774;margin-top:2px'>"
            f"{data['completed']}/{data['total']} tasks</div>",
            unsafe_allow_html=True,
        )
        col_pct.markdown(
            f"<div style='font-size:13px;font-weight:600;color:#1a1a1a;margin-top:2px'>"
